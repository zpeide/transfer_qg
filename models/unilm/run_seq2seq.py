    ...
    if args.num_training_steps <= global_step:
        ...
        for step, batch in enumerate(train_iterator):
            ...
            if not args.no_lr_schedule:
                ...
                train_iterator.set_description('Iter (loss=%5.3f) lr=%9.7f' % (loss.item(), args.learning_rate))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and \
                        (global_step % args.save_steps == 0 or global_step == args.num_training_steps or 
                         global_step % (len(training_features) // train_batch_size) == 0 ):

                    save_path = os.path.join(args.output_dir, "ckpt-%d" % global_step)
                    if global_step % (len(training_features) // train_batch_size) == 0:
                        _epoch_i = global_step // (len(training_features) // train_batch_size)
                        save_path = os.path.join(args.output_dir, "epoch-%d" % _epoch_i)
...
def get_args():
    ...
    parser.add_argument("--no_lr_schedule", action='store_true', default=False,
                        help="don't change the learning rate during training.")
...
def main():
    ...
    training_features = utils.load_and_cache_examples(
        ...
        cached_features_file=args.cached_train_features_file, shuffle=not args.no_shuffle,
    )
