...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        # back tracking
        traces = {'pred_seq': [], 'scores': [], 'wids': [], 'ptrs': [], 'pred_seq_scores': []}
        for b in range(batch_size):
            # [(beam,)]
            ...
            if frame_id == -1:
                ...
                seq_scores = [scores[frame_id][pos_in_frame]]
                for fid in range(frame_id, 0, -1):
                    pos_in_frame = ptrs[fid][pos_in_frame]
                    seq.append(wids_list[fid - 1][pos_in_frame])
                    seq_scores.append(scores[fid - 1][pos_in_frame])
                seq.reverse()
                seq_scores.reverse()
                traces['pred_seq'].append(seq)
                traces['pred_seq_scores'].append(torch.tensor(seq_scores, dtype=torch.float32))
        ...
        for k in ('pred_seq', 'scores', 'wids', 'ptrs', 'pred_seq_scores'):
            ts_list = traces[k]
            if not isinstance(ts_list[0], torch.Tensor):
                dt = torch.float if k in ['scores'] else torch.long
