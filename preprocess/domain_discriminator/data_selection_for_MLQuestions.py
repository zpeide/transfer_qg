...
file_paths_new = {
                ...
                'MLQ':  os.path.join(base_path_new, 'MLQuestions/target.jsonl'),
            }
...
mlq_ds = [' '.join(json.loads(_)['text']).lower() for _ in open(file_paths_new['MLQ'])]
...
nq_features = torch.load("features-bert-base-uncased.pt")['NQ']
if os.path.exists(f'features-{models_to_use[0]}.mlq-targetset.pt'):
    features = torch.load(f'features-{models_to_use[0]}.mlq-targetset.pt')
else:
    ...
    with torch.no_grad():
        for domain, ds in {'MLQ':mlq_ds}.items():
            # items = list(set(ds))
            items = ds
            avg_pool = []
            cls_features = []
    ...
    torch.save(features, f'features-{models_to_use[0]}.mlq-targetset.pt')
features['NQ'] = nq_features

def make_clustering( corpus_used=['NQ', 'MLQ'], num_samples_for_clustering = 3000):
...
#make_clustering(corpus_used=['NQ', 'MLQ'], num_samples_for_clustering=3000)
other_center_l2_dis = torch.load("results_NQ-MLQ.target.pt")['avg']['gmm']['other_center_l2_distance']

nq_mlq_dis = other_center_l2_dis[:len(nq_ds)]
nq_mlq_sorted_idx = np.argsort(nq_mlq_dis)

nq_ds = [json.loads(_) for _ in open(file_paths_new['NQ'])]

for i in range(0, 10000, 1000):
    e = min(i+1000, len(nq_ds))
    f = open(f"../../data/nq/mlq_order/{e}.jsonl", "w")
    for j in range(0, e):
        item = nq_ds[nq_mlq_sorted_idx[j]]
        item['race_dis'] = other_center_l2_dis[nq_mlq_sorted_idx[j]]
