...
if os.path.exists(f'features-{models_to_use[0]}.pt'):
    features = torch.load(f'features-{models_to_use[0]}.pt')
else:
    ...
    torch.save(features, f'features-{models_to_use[0]}.pt')
...
def make_clustering( corpus_used=['NQ', 'RACE', 'SciQ'], num_samples_for_clustering = 3000):
    ...
    for pool_type, pool_states_id in states_used.items():
        ...
        plt.savefig(f"clustering_{pool_type}_confusion_gmm_{trial_name}.pdf", dpi=600, bbox_inches='tight')
        ...
        plt.savefig(f"clustering_{pool_type}_confusion_gmm_cosine_{trial_name}.pdf", dpi=600, bbox_inches='tight')
        ...
        plt.savefig(f"{pool_type}_confusion_gmm_{trial_name}.pdf", dpi=600, bbox_inches='tight')
        ...
        plt.savefig(f"clustering_{pool_type}_confusion_minikm_{trial_name}.pdf", dpi=600, bbox_inches='tight')
        ...
        plt.savefig(f"{pool_type}_confusion_minikm_{trial_name}.pdf", dpi=600, bbox_inches='tight')
    ...
    torch.save(final_results, f'results_{trial_name}.pt')


# make_clustering(corpus_used=['NQ', 'RACE'], num_samples_for_clustering=5000)
